(() => {
  ...
  const experiencePercent = $('#experiencePercent');
  ...
  // Preload the complete five-angle sequence for seamless scroll rotation.
  flavours.forEach(f=>{
    for(let i=0;i<5;i++){
      const image=new Image();
      image.src=`assets/images/${f.key}-${String(i).padStart(3,'0')}.webp`;
    }
  });
  ...
  function onScroll(){
    ...
    requestAnimationFrame(()=>{
      ...
      root.style.setProperty('--halo-scale', String(.72 + p * .24));
      root.style.setProperty('--tunnel-rotate', `${p * 48}deg`);
      root.style.setProperty('--product-shift', `${Math.sin(p * Math.PI * 1.35) * (innerWidth <= 900 ? 2.2 : 6.5)}vw`);
      root.style.setProperty('--product-rotate', `${(-5 + p * 10 + Math.sin(p * Math.PI * 2) * 2.2).toFixed(2)}deg`);
      root.style.setProperty('--orbit-rotate', `${p * 240}deg`);
      root.style.setProperty('--product-scale', String(.9 + Math.sin(p * Math.PI) * .12 + p * .035));
      root.style.setProperty('--product-lift', `${Math.sin(p * Math.PI * 2) * -9}px`);
      experienceMeter.style.transform=`scaleX(${p})`;
      experiencePercent.textContent=`${String(Math.round(p*100)).padStart(3,'0')}%`;
      const framePosition = p * (sequenceFrames.length - 1);
      sequenceFrames.forEach((frame,i)=>{
        const distance = Math.abs(i-framePosition);
        const opacity = Math.max(0,1-distance);
        frame.style.opacity = opacity.toFixed(3);
        frame.style.transform = `scale(${(.975 + opacity*.025).toFixed(3)}) translateY(${((1-opacity)*5).toFixed(1)}px)`;
        frame.style.filter = `blur(${((1-opacity)*1.2).toFixed(2)}px)`;
        frame.style.zIndex = String(Math.round(opacity*10));
      });
  ...
  // Subtle cinematic parallax. It remains intentionally small so the product stays stable on phones.
  let pointerRaf = 0;
  function setPointerMotion(x,y){
    if(pointerRaf) cancelAnimationFrame(pointerRaf);
    pointerRaf=requestAnimationFrame(()=>{
      const nx=clamp(x/innerWidth,0,1)*2-1;
      const ny=clamp(y/innerHeight,0,1)*2-1;
      root.style.setProperty('--mx',nx.toFixed(3));
      root.style.setProperty('--my',ny.toFixed(3));
      root.style.setProperty('--experience-tilt-x',`${(-ny*2.2).toFixed(2)}deg`);
      root.style.setProperty('--experience-tilt-y',`${(nx*3.2).toFixed(2)}deg`);
    });
  }
  addEventListener('pointermove',e=>setPointerMotion(e.clientX,e.clientY),{passive:true});
  addEventListener('deviceorientation',e=>{
    if(e.gamma==null||e.beta==null)return;
    const x=clamp((e.gamma+25)/50)*innerWidth;
    const y=clamp((e.beta-15)/70)*innerHeight;
    setPointerMotion(x,y);
  },{passive:true});
